...
export default function LandMap({ lands, onLandClick, isLoading, isMiningLocked = false }: LandMapProps) {
  ...
  conditionalLog('土地地图: 渲染地图，土地数量:', lands.length, '挖矿锁定:', isMiningLocked);
  ...
  if (isLoading) {
    return (
      <div className="bg-dark-surface rounded-2xl border-3 border-gray-500 p-6 text-center">
        ...
        <p className="text-dark-text font-bold global-font">加载土地地图中...</p>
      </div>
    );
  }
    ...
    <div className="bg-dark-surface rounded-2xl border-3 border-gray-500 p-6">
      <div className="mb-6">
        <div className="flex items-center space-x-3 mb-4">
          ...
          <div>
            <h3 className="text-lg font-bold text-dark-text global-font">土地地图</h3>
        ...
        <div className="flex flex-wrap gap-3 text-sm mb-4">
          <div className="flex items-center space-x-2">
            ...
            <span className="text-dark-text-secondary global-font">空置土地（可挖矿）</span>
          </div>
          {isMiningLocked && (
            <div className="flex items-center space-x-2">
              <div className="w-4 h-4 bg-gray-500 rounded border border-gray-600"></div>
              <span className="text-dark-text-secondary global-font">操作禁用</span>
            </div>
          )}
        </div>
      </div>
            ...
            Array.from({ length: GRID_WIDTH }, (_, x) => {
              ...
              return (
                <button
                  ...
                  title={`土地 #${landId} (${x}, ${y}) - ${
                    isMiningLocked ? '角色死亡或挖矿中，无法操作' : '空置土地，可挖矿'
                  }`}
                  disabled={isDisabled}
                  type="button"
                >
      ...
      <div className="mt-4 bg-gradient-to-r from-theme-primary/20 to-theme-secondary/20 rounded-xl p-3 border border-gray-500">
        <div className="flex items-center space-x-2">
          ...
          <div>
            <p className="text-dark-text font-bold global-font text-xs">游戏挖矿系统</p>
