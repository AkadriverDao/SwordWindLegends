...
import { ArrowLeftIcon, TrendingUpIcon, BookOpenIcon, UsersIcon, HeartIcon, ZapIcon, CoinsIcon, PickaxeIcon } from 'lucide-react';
import CryptocurrencyList from './CryptocurrencyList';
import CommentSection from './CommentSection';
import ArticleList from './ArticleList';
import { useGetAllTradingPairs, useGetArticlesByCategory } from '../hooks/useQueries';

interface CategoryPageProps {
  categoryName?: string;
  navigate: (route: '/' | '/profile' | '/leaderboard' | '/customize' | '/article' | '/management' | '/category', params?: any) => void;
}
...
const categoryInfo: Record<string, {
  ...
  '加密趋势': {
    ...
    emoji: '₿',
    content: '这里将展示与加密货币、区块链技术、DeFi、NFT等相关的最新趋势和深度分析文章。'
  },
  '技术分享': {
    icon: ZapIcon,
    ...
    emoji: '💻',
    content: '程序员和开发者分享编程技巧、框架使用心得、项目经验和技术见解的专区。'
  },
  '人生经历': {
    icon: HeartIcon,
  ...
  'NFT市场': {
    ...
    emoji: '🎨',
    content: '探索NFT艺术品、收藏品交易，了解数字资产市场动态和投资机会。'
  },
  '土地矿工': {
    icon: PickaxeIcon,
    ...
    emoji: '⛏️',
    content: '分享虚拟土地投资、区块链游戏挖矿经验和GameFi项目体验。'
  }
};
...
export default function CategoryPage({ categoryName, navigate }: CategoryPageProps) {
  const { data: backendTradingPairs, isLoading: tradingPairsLoading } = useGetAllTradingPairs();
  const { data: categoryArticles, isLoading: articlesLoading } = useGetArticlesByCategory(categoryName || '');
    ...
    <div className="max-w-6xl mx-auto">
      <div className="bg-dark-card backdrop-blur-sm rounded-3xl shadow-2xl border-4 border-gray-500">
          ...
          <div className="flex items-center space-x-4">
            ...
            <div className="flex-1">
              <div className="flex items-center space-x-3 mb-2">
                <div className={`w-12 h-12 bg-gradient-to-r ${category.color} rounded-full flex items-center justify-center shadow-lg`}>
              ...
              <p className="text-dark-text-secondary font-medium global-font">{category.description}</p>
            </div>
          </div>
        </div>
        ...
        <div className="p-8 space-y-8">
          ...
          <div className={`bg-gradient-to-r ${category.color} rounded-2xl p-8 text-white border-3 border-gray-500`}>
            ...
            <div className="space-y-8">
              ...
              {tradingPairsLoading ? (
                <div className="bg-dark-surface rounded-2xl p-8 border-3 border-gray-500 text-center">
                  ...
                  <p className="text-dark-text text-xl font-bold global-font">加载交易对配置中...</p>
                </div>
                    ...
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        ...
                        <p className="text-dark-text-secondary text-xs global-font">
                          包括 ICP/USDT 在内的所有交易对都需要超级管理员在管理系统中配置并启用后才会显示。这确保了所有用户看到的交易对列表完全一致。
                        </p>
                      </div>
                    </div>
                  </div>
                </div>
              ) : backendTradingPairs.filter(([_, pair]) => pair.enabled).length === 0 ? (
                <div className="bg-dark-surface rounded-2xl p-8 border-3 border-gray-500 text-center">
                  <div className="w-20 h-20 bg-gradient-to-br from-dark-warning to-dark-accent rounded-full flex items-center justify-center mx-auto mb-6 shadow-lg">
                    <span className="text-white text-3xl">🔒</span>
                  </div>
          ...
          {/* Category Articles Section for 技术分享, 学习笔记, 人生经历, NFT市场, and 土地矿工 with Flexible Approval Logic */}
          {(categoryName === '技术分享' || categoryName === '学习笔记' || categoryName === '人生经历' || categoryName === 'NFT市场' || categoryName === '土地矿工') && (
            <div className="space-y-6">
              <div className="flex items-center justify-between">
                ...
                <div className="text-dark-text-secondary text-sm">
                  {articlesLoading ? '加载中...' : `共 ${categoryArticles?.length || 0} 篇文章`}
                </div>
              </div>
              ...
              {articlesLoading ? (
                <div className="bg-dark-surface rounded-2xl p-8 border-3 border-gray-500 text-center">
                  <div className="animate-spin rounded-full h-12 w-12 border-4 border-theme-primary border-t-transparent mx-auto mb-6"></div>
                  <p className="text-dark-text text-xl font-bold global-font">加载文章中...</p>
                </div>
              ) : categoryArticles && categoryArticles.length > 0 ? (
                <>
                  {/* Information about flexible approval logic */}
                  <div className="bg-gradient-to-r from-theme-primary/20 to-theme-secondary/20 rounded-xl p-4 border-2 border-gray-500 mb-6">
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        ...
                        <p className="text-dark-text-secondary text-xs global-font">
                          分类页面显示所有带有"{categoryName}"标签的文章，包括未审核、审核中和已审核的文章。只有已审核通过的文章才会在主页显示。
                        </p>
                      </div>
                    </div>
                  </div>
                ...
                <div className="bg-dark-surface rounded-2xl p-8 border-3 border-gray-500 text-center">
                  ...
                  <p className="text-dark-text-secondary text-lg mb-6 global-font">
                    还没有用户发布{categoryName}类型的文章，快来成为第一个分享者吧！
                  </p>
                  <div className="bg-gradient-to-r from-theme-primary/20 to-theme-secondary/20 rounded-xl p-4 border-2 border-gray-500">
                    <div className="flex items-center space-x-3">
                      <span className="text-2xl">💡</span>
                      <div>
                        ...
                        <p className="text-dark-text-secondary text-xs global-font">
                          在个人页面创作文章时，选择"{categoryName}"分类标签。文章创建后即可在此分类页面中查看，无需等待审核。
                        </p>
                      </div>
                    </div>
                  </div>
          ...
          {categoryName !== '加密趋势' && categoryName !== '技术分享' && categoryName !== '学习笔记' && categoryName !== '人生经历' && categoryName !== 'NFT市场' && categoryName !== '土地矿工' && (
            <div className="bg-dark-surface rounded-2xl p-8 border-3 border-gray-500 text-center">
              <div className="w-20 h-20 bg-gradient-to-br from-dark-primary to-dark-secondary rounded-full flex items-center justify-center mx-auto mb-6 shadow-lg">
                <span className="text-white text-3xl">🚧</span>
              </div>
              ...
              <p className="text-dark-text-secondary text-lg mb-6 global-font">
                {categoryName} 专区的内容正在精心准备中，敬请期待更多精彩内容！
              </p>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-8">
                <div className="bg-dark-card rounded-xl p-6 border-2 border-gray-500">
                  <div className="flex items-center space-x-3 mb-3">
                    <span className="text-2xl">📝</span>
                    <h4 className="text-lg font-bold text-dark-text global-font">专题文章</h4>
                  </div>
                ...
                <div className="bg-dark-card rounded-xl p-6 border-2 border-gray-500">
                  <div className="flex items-center space-x-3 mb-3">
                    <span className="text-2xl">💬</span>
                    <h4 className="text-lg font-bold text-dark-text global-font">互动讨论</h4>
                  </div>
                  <p className="text-dark-text-secondary text-sm global-font">
                ...
                <div className="bg-dark-card rounded-xl p-6 border-2 border-gray-500">
                  <div className="flex items-center space-x-3 mb-3">
                    ...
                    <h4 className="text-lg font-bold text-dark-text global-font">数据分析</h4>
                  </div>
                  <p className="text-dark-text-secondary text-sm global-font">
                ...
                <div className="bg-dark-card rounded-xl p-6 border-2 border-gray-500">
                  <div className="flex items-center space-x-3 mb-3">
                    <span className="text-2xl">🎯</span>
                    <h4 className="text-lg font-bold text-dark-text global-font">精选推荐</h4>
                  </div>
                  <p className="text-dark-text-secondary text-sm global-font">
