...
import { useDeleteComment, type CommentWithReplies } from '../../hooks/useQueries';
import { useInternetIdentity } from '../../hooks/useInternetIdentity';
import { UserIcon, ReplyIcon, ChevronDownIcon, ChevronRightIcon, TrashIcon } from 'lucide-react';
...
export default function CommentItem({ comment, articleId, level = 0, isAuthenticated }: CommentItemProps) {
  ...
  const canDelete = isAuthenticated && identity && comment.author.toString() === identity.getPrincipal().toString();
  ...
  const borderLeftClass = level > 0 ? 'border-l-2 border-theme-primary/30 pl-4' : '';
          ...
          <div className="bg-dark-card backdrop-blur-sm rounded-3xl shadow-2xl border-4 border-dark-error w-full max-w-md">
            <div className="p-6 border-b-4 border-gray-500 bg-gradient-to-r from-dark-error/20 to-dark-warning/20 rounded-t-3xl">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <h3 className="text-xl font-bold text-dark-text">⚠️ 确认删除评论</h3>
                  <p className="text-dark-text-secondary font-medium">此操作无法撤销</p>
                </div>
              </div>
            </div>
