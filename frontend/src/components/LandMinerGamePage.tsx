...
export default function LandMinerGamePage({ navigate }: LandMinerGamePageProps) {
  ...
  useEffect(() => {
    if (landsError) {
      ...
      setBackendConnectionError('无法连接到游戏服务器，请检查网络连接或联系管理员');
  ...
  }, [landsError, actor, actorFetching, loggingEnabled]);
  ...
  const handleMineLand = async (landId: bigint): Promise<void> => {
    ...
    try {
      ...
      setBackendConnectionError(error instanceof Error ? error.message : '挖矿操作失败');
      throw error;
    }
  };
  ...
  const handleReviveCharacter = async (): Promise<void> => {
    ...
    try {
      ...
      conditionalLog('土地矿工游戏: 角色复活完成，UI状态已重置，游戏数据已刷新');
      
    } catch (error) {
      conditionalLog('土地矿工游戏: 复活失败:', error);
      setBackendConnectionError(error instanceof Error ? error.message : '复活操作失败');
      throw error;
    }
  };
        ...
        <div className="p-8">
                  ...
                  <div className="flex items-center space-x-4">
                    ...
                    <div>
                      ...
                      <p className="text-white/90 text-lg global-font">
                        土地 #{selectedLand?.id.toString()} - 等待系统生成挖矿结果
                      </p>
                    </div>
                  </div>
                  <div className="text-center">
                    ...
                    <p className="text-white/80 text-sm global-font">秒后获得结果</p>
                  </div>
                </div>
          ...
          <div className="mt-8 bg-gradient-to-r from-theme-primary/20 to-theme-secondary/20 rounded-xl p-6 border-2 border-gray-500">
            <div className="flex items-start space-x-3">
              ...
              <div>
                <p className="text-dark-text font-bold global-font text-sm mb-2">游戏系统说明</p>
