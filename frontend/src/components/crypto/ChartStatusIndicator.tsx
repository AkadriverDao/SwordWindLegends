    ...
    <div className="absolute top-4 left-4 bg-dark-surface/80 backdrop-blur-sm rounded-lg px-3 py-2 border border-gray-500">
      <div className="flex items-center space-x-2">
        <div className={`w-2 h-2 rounded-full ${
          cacheStatus === 'cached' ? 'bg-dark-success' :
          cacheStatus === 'refreshing' ? 'bg-dark-warning animate-pulse' :
          'bg-dark-error'
        }`}></div>
        <span className="text-dark-text-secondary text-xs font-medium global-font">
          {cacheStatus === 'cached' && `${selectedTradingPair} 缓存数据 (10秒刷新)`}
          {cacheStatus === 'refreshing' && `${selectedTradingPair} 后台刷新中...`}
          {cacheStatus === 'unavailable' && `${selectedTradingPair} ${error || '数据暂时获取不到'}`}
        </span>
      </div>
    </div>
  );
  ...
  return (
    <div
      ...
      style={{
        ...
        transform: hoveredPoint.x > containerRef.current.clientWidth - 250 ? 'translateX(-100%)' : 'none'
      }}
    >
