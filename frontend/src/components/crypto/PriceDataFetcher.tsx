...
import coinGeckoService, { type CoinGeckoPrice } from '../../services/coingecko';
...
export default function PriceDataFetcher({ selectedTradingPair, onDataUpdate }: PriceDataFetcherProps) {
  const [currentSymbol, setCurrentSymbol] = useState<string>(selectedTradingPair);

  // Enhanced data fetching with cache-first strategy and strict symbol isolation
  const fetchPriceData = useCallback(async () => {
    try {
      console.log(`从缓存优先获取 ${selectedTradingPair} 数据，确保严格符号隔离`);
      
      // Get cache status first with strict symbol validation
      const dataStatus = coinGeckoService.getCacheDataStatus(selectedTradingPair);
      ...
      if (!supportedPairs.includes(selectedTradingPair)) {
        console.warn(`不支持的交易对: ${selectedTradingPair}，支持的交易对: ${supportedPairs.join(', ')}`);
        onDataUpdate({
          priceData: [],
          currentPrice: null,
          priceChange: null,
      ...
      const isValidPair = await coinGeckoService.validateTradingPair(selectedTradingPair);
      if (!isValidPair) {
      ...
      if (selectedTradingPair !== currentSymbol) {
        console.log(`交易对已切换，忽略 ${selectedTradingPair} 的数据以确保符号隔离`);
        return;
      }
      ...
      if (chartData && Array.isArray(chartData) && chartData.length > 0) {
        ...
        if (validData.length > 0) {
          ...
          if (latest && earliest && latest.price > 0 && earliest.price > 0) {
            newPriceChange = ((latest.price - earliest.price) / earliest.price) * 100;
          }
          
          console.log(`立即显示 ${validData.length} 个缓存图表数据点，币种: ${selectedTradingPair}，确保符号隔离`);
        } else {
          console.log(`${selectedTradingPair} 图表数据格式无效，显示空图表以确保符号隔离`);
          newIsInitialLoading = true;
        }
      } else {
        console.log(`${selectedTradingPair} 暂无图表数据，显示空图表以确保符号隔离`);
        newIsInitialLoading = true;
      }
      
      // Handle current price data with strict symbol validation
      if (priceDataResult && typeof priceDataResult.price === 'number' && priceDataResult.price > 0) {
        newCurrentPrice = priceDataResult.price;
        console.log(`立即显示 ${selectedTradingPair} 缓存当前价格: $${priceDataResult.price}，确保符号隔离`);
      } else {
        console.log(`${selectedTradingPair} 暂无当前价格数据，清空价格显示以确保符号隔离`);
      }
      ...
      try {
        const cachedChartData = await coinGeckoService.getHistoricalData(selectedTradingPair);
        const cachedPriceData = await coinGeckoService.getCurrentPrice(selectedTradingPair);
        
        if (selectedTradingPair === currentSymbol) {
          let cachedPriceDataArray: CoinGeckoPrice[] = [];
          let cachedCurrentPrice: number | null = null;
          
          if (cachedChartData && Array.isArray(cachedChartData) && cachedChartData.length > 0) {
            cachedPriceDataArray = cachedChartData;
            console.log(`显示 ${selectedTradingPair} 缓存数据，后台刷新失败，确保符号隔离`);
          }
      ...
      console.log(`${selectedTradingPair} 显示缓存数据或"数据暂时获取不到"状态，确保符号隔离`);
    }
  }, [selectedTradingPair, currentSymbol, onDataUpdate]);

  // Reset states when trading pair changes
  useEffect(() => {
