...
class CoinGeckoService {
  ...
  private async loadConfigurationFromBackend(): Promise<void> {
    ...
    try {
      ...
      for (const [symbol, tradingPair] of backendTradingPairs) {
        ...
        console.log(`配置交易对: ${symbol} -> ${tradingPair.coinId} (${tradingPair.enabled ? '启用' : '禁用'})`);
      }

      this.configurationLoaded = true;
      console.log(`成功加载 ${this.tradingPairConfigs.size} 个完整交易对配置，包括 coinId 映射`);

      // Execute any pending callbacks
      this.configurationCallbacks.forEach(callback => callback());
  ...
  private saveCacheToStorage(): void {
    try {
      ...
      if (totalSize > this.MAX_STORAGE_SIZE) {
        ...
        localStorage.setItem(priceStorageKey, JSON.stringify(cleanedPriceCache));
        localStorage.setItem(chartStorageKey, JSON.stringify(cleanedChartCache));
      } else {
      ...
      if (error instanceof DOMException && error.name === 'QuotaExceededError') {
        ...
        try {
          ...
          localStorage.setItem(`${this.STORAGE_KEY_PREFIX}${this.CACHE_VERSION}_price`, JSON.stringify(priceCache));
          localStorage.setItem(`${this.STORAGE_KEY_PREFIX}${this.CACHE_VERSION}_chart`, JSON.stringify(chartCache));
        } catch (retryError) {
  ...
  private evictOldestCacheEntries(): void {
    const allEntries: Array<{ key: string; timestamp: number; type: 'price' | 'chart' }> = [];
  ...
  private isCacheValid<T>(cache: Map<string, CacheEntry<T>>, cacheKey: string): boolean {
  ...
  private hasCachedData<T>(cache: Map<string, CacheEntry<T>>, cacheKey: string, expectedTradingPair: string): boolean {
    const cached = cache.get(cacheKey);
    if (!cached) return false;
    ...
    if (cached.tradingPair !== expectedTradingPair) {
      console.warn(`缓存数据交易对不匹配: 期望 ${expectedTradingPair}, 实际 ${cached.tradingPair}`);
      cache.delete(cacheKey);
      this.saveCacheToStorage();
      return false;
    }
  ...
  private async processStaggeredFetch(): Promise<void> {
    if (this.staggeredFetchInProgress || this.staggeredFetchQueue.length === 0) {
      return;
    }
  ...
  private async processRequestQueue<T>(tradingPair: string, requestFn: () => Promise<T>): Promise<void> {
        ...
        try {
          ...
          if (lastRequest) {
            ...
            if (timeSinceLastRequest < this.REQUEST_INTERVAL) {
              ...
              console.log(`${tradingPair} 后台刷新等待 ${Math.ceil(waitTime / 1000)} 秒`);
  ...
  private async makeRequestWithRetry<T>(
    ...
    try {
      ...
      if (!response.ok) {
        if (response.status === 429) {
          ...
          if (retryCount < this.MAX_RETRIES) {
            ...
            console.warn(`${tradingPair} 后台API请求频率超限，${delay}ms 后重试 (尝试 ${retryCount + 1}/${this.MAX_RETRIES})`);
            await this.sleep(delay);
            return this.makeRequestWithRetry<T>(endpoint, params, tradingPair, retryCount + 1);
          }
          throw new Error(`${tradingPair} 后台API请求频率超限，请稍后重试`);
        } else if (response.status >= 500 && retryCount < this.MAX_RETRIES) {
          // Server error, retry with exponential backoff
          const delay = this.RETRY_DELAY * Math.pow(2, retryCount);
          console.warn(`${tradingPair} 后台服务器错误 ${response.status}，${delay}ms 后重试 (尝试 ${retryCount + 1}/${this.MAX_RETRIES})`);
          await this.sleep(delay);
          return this.makeRequestWithRetry<T>(endpoint, params, tradingPair, retryCount + 1);
        } else {
          throw new Error(`${tradingPair} 后台API请求失败: ${response.status} ${response.statusText}`);
        }
      }
      ...
      if (!data || typeof data !== 'object') {
        throw new Error(`${tradingPair} 后台API返回数据格式无效：数据不是有效的对象`);
      }
      ...
      if (error instanceof Error) {
        if (error.name === 'AbortError' || error.message.includes('timeout')) {
          if (retryCount < this.MAX_RETRIES) {
            console.warn(`${tradingPair} 后台请求超时，重试中 (尝试 ${retryCount + 1}/${this.MAX_RETRIES})`);
  ...
  public async getCurrentPrice(tradingPair: string = 'ICP/USDT'): Promise<CoinGeckoPrice | null> {
    // Wait for configuration to be loaded
    ...
    if (!coinId) {
      console.warn(`不支持的交易对: ${tradingPair}，支持的交易对: ${this.getSupportedTradingPairs().join(', ')}`);
      return null;
    }
    ...
    if (cached && cached.tradingPair === tradingPair) {
      ...
      if (!this.isCacheValid(this.priceCache, cacheKey) && !this.backgroundRefreshInProgress.has(`price_${tradingPair}`)) {
    ...
    return this.queueRequest(tradingPair, 'price', () => this.fetchFreshPriceData(tradingPair, coinId));
  }

  // Background fetch for cache refresh
  private async fetchFreshPriceDataBackground(tradingPair: string, coinId: string): Promise<void> {
    try {
      await this.queueRequest(tradingPair, 'price', () => this.fetchFreshPriceData(tradingPair, coinId));
  ...
  private async fetchFreshPriceDataStaggered(tradingPair: string): Promise<void> {
    const coinId = this.getCoinId(tradingPair);
    if (!coinId) return;
  ...
  private async fetchFreshPriceData(tradingPair: string, coinId: string): Promise<CoinGeckoPrice | null> {
    ...
    try {
      console.log(`后台获取 ${tradingPair} 当前价格，使用后端提供的币种ID: ${coinId}`);
      
      const response = await this.makeRequestWithRetry<CoinGeckoPriceResponse>('/simple/price', params, tradingPair);
      ...
      if (typeof coinData.usd !== 'number' || coinData.usd <= 0 || !isFinite(coinData.usd)) {
      ...
      const priceData: CoinGeckoPrice = {
        ...
        price: coinData.usd,
        volume: coinData.usd_24h_vol && isFinite(coinData.usd_24h_vol) && coinData.usd_24h_vol > 0 ? coinData.usd_24h_vol : undefined,
        market_cap: coinData.usd_market_cap && isFinite(coinData.usd_market_cap) && coinData.usd_market_cap > 0 ? coinData.usd_market_cap : undefined,
      };
      ...
      console.log(`${tradingPair} 后台当前价格获取成功: $${coinData.usd}，使用后端 coinId: ${coinId}`);
      return priceData;
    } catch (error) {
  ...
  public async getHistoricalData(tradingPair: string = 'ICP/USDT'): Promise<CoinGeckoPrice[] | null> {
    // Wait for configuration to be loaded
    await this.waitForConfiguration();
    ...
    if (cached && cached.tradingPair === tradingPair) {
      ...
      // Start background refresh if cache is getting old or invalid
      if (!this.isCacheValid(this.chartCache, cacheKey) && !this.backgroundRefreshInProgress.has(`chart_${tradingPair}`)) {
    ...
    return this.queueRequest(tradingPair, 'chart', () => this.fetchFreshChartData(tradingPair));
  }

  // Background fetch for expired chart cache
  private async fetchFreshChartDataBackground(tradingPair: string): Promise<void> {
    try {
      await this.queueRequest(tradingPair, 'chart', () => this.fetchFreshChartData(tradingPair));
  ...
  private async fetchFreshChartDataStaggered(tradingPair: string): Promise<void> {
    try {
  ...
  private async fetchFreshChartData(tradingPair: string): Promise<CoinGeckoPrice[] | null> {
    const coinId = this.getCoinId(tradingPair);
    ...
    try {
      ...
      console.log(`后台获取 ${tradingPair} CoinGecko 固定30天历史数据，使用后端提供的币种ID: ${coinId}`);

      const response = await this.makeRequestWithRetry<CoinGeckoMarketChartRangeResponse>(
      ...
      if (!response.prices || !Array.isArray(response.prices) || response.prices.length === 0) {
      ...
      const validPrices = response.prices.filter(([timestamp, price]) => {
        return Array.isArray([timestamp, price]) &&
      ...
      const historicalData: CoinGeckoPrice[] = validPrices.map(([timestamp, price], index) => {
        ...
        return {
          timestamp,
          price,
          volume: volume && isFinite(volume) && volume > 0 ? volume : undefined,
          market_cap: marketCap && isFinite(marketCap) && marketCap > 0 ? marketCap : undefined,
        };
      });
      ...
      if (historicalData.length < 10) {
        console.warn(`${tradingPair} 后台历史数据点过少: ${historicalData.length}`);
      }

      console.log(`${tradingPair} 后台成功获取 ${historicalData.length} 个真实数据点，固定30天时间范围，使用后端 coinId: ${coinId}`);
  ...
  private getFixed30DayConfig(): { fromTimestamp: number; toTimestamp: number } {
  ...
  public async validateTradingPair(tradingPair: string): Promise<boolean> {
    ...
    try {
      ...
      const coinData = response[coinId];
      if (!coinData || typeof coinData !== 'object') {
        console.warn(`${tradingPair} 验证失败：未找到币种数据，币种ID: ${coinId}`);
        return false;
      }
      ...
      console.log(`${tradingPair} 交易对验证成功，使用后端提供的币种ID: ${coinId}`);
      return true;
    } catch (error) {
  ...
  public clearCache(tradingPair?: string): void {
    if (tradingPair) {
      ...
      this.getSupportedTradingPairs().forEach(pair => {
  ...
  public getCacheStatus(): { 
    ...
    let storageUsage = '未知';
    try {
      const priceCache = JSON.stringify(Object.fromEntries(this.priceCache.entries()));
      const chartCache = JSON.stringify(Object.fromEntries(this.chartCache.entries()));
  ...
  public async fetchAllTradingPairs(tradingPairs: string[]): Promise<void> {
    console.log('立即批量获取所有交易对缓存数据，启动分批后台刷新:', tradingPairs);
    
    // Wait for configuration to be loaded
    await this.waitForConfiguration();
    
    // Filter to only supported and enabled trading pairs
    const supportedPairs = tradingPairs.filter(pair => this.getCoinId(pair) && this.isTradingPairEnabled(pair));
    const unsupportedPairs = tradingPairs.filter(pair => !this.getCoinId(pair));
    const disabledPairs = tradingPairs.filter(pair => this.getCoinId(pair) && !this.isTradingPairEnabled(pair));
    
    if (unsupportedPairs.length > 0) {
      console.warn('发现不支持的交易对:', unsupportedPairs, '支持的交易对:', this.getSupportedTradingPairs());
    }
    ...
    const cachedPairs = cacheResults.filter(r => r.hasPrice && r.hasChart).length;
    console.log(`立即从缓存提供 ${cachedPairs}/${supportedPairs.length} 个交易对数据`);
  ...
  public getCacheDataStatus(tradingPair: string): {
    ...
    const priceAge = (hasPrice && priceCache) ? Date.now() - priceCache.timestamp : undefined;
    const chartAge = (hasChart && chartCache) ? Date.now() - chartCache.timestamp : undefined;
    
    const isRefreshing = this.backgroundRefreshInProgress.has(`price_${tradingPair}`) || 
