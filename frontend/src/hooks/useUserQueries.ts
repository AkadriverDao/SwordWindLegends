...
import type { UserProfile, MiningLeaderboardEntry } from '../backend';
...
export function useIsSuperUser() {
  ...
  const isSuperUser = identity ? SUPER_USER_PRINCIPALS.includes(identity.getPrincipal().toString()) : false;
...
export function useHasManagementAccess() {
  ...
  const hasManagementAccess = identity ? SUPER_USER_PRINCIPALS.includes(identity.getPrincipal().toString()) : false;
