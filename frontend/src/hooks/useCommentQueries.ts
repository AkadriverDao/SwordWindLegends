...
async function buildCommentHierarchy(comments: Comment[], actor: any): Promise<CommentWithReplies[]> {
  const userProfileMap = new Map<string, UserProfile | null>();
  
  const uniqueAuthors = [...new Set(comments.map(comment => comment.author.toString()))];
  
  await Promise.all(
    uniqueAuthors.map(async (authorPrincipal) => {
      try {
        const profile = await actor.getUserProfile(Principal.fromText(authorPrincipal));
        userProfileMap.set(authorPrincipal, profile);
      } catch (error) {
        await conditionalLog(actor, 'Error fetching user profile for', authorPrincipal, error);
  ...
  return useQuery<CommentWithReplies[]>({
    ...
    queryFn: async () => {
      ...
      try {
        ...
        const hierarchicalComments = await buildCommentHierarchy(comments, actor);
        return hierarchicalComments.sort((a, b) => Number(b.createdAt - a.createdAt));
      } catch (error) {
...
export function useCreateReply() {
  ...
  return useMutation({
    mutationFn: async ({ articleId, parentComment, content }: { 
      articleId: string; 
...
export function useDeleteComment() {
  ...
  return useMutation({
    mutationFn: async (commentId: bigint) => {
      ...
      await conditionalLog(actor, '当前用户主体ID:', identity.getPrincipal().toString());
  ...
  return useQuery<Comment[]>({
    ...
    queryFn: async () => {
      ...
      try {
        ...
        const comments = await actor.getCommentsForArticle(CRYPTO_TRENDS_ARTICLE_ID);
...
export function useDeleteCryptoTrendsComment() {
  ...
  return useMutation({
    mutationFn: async (commentId: bigint) => {
      ...
      await conditionalLog(actor, '正在删除加密趋势评论，ID:', commentId.toString());
      await conditionalLog(actor, '当前用户主体ID:', identity.getPrincipal().toString());
