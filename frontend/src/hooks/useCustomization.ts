...
import { BlogCustomization, DEFAULT_CUSTOMIZATION } from '../types/customization';

const STORAGE_KEY_PREFIX = 'blog_customization_';

export function useCustomization() {
  const { identity } = useInternetIdentity();
  const [customization, setCustomization] = useState<BlogCustomization>(DEFAULT_CUSTOMIZATION);
  ...
  const saveCustomization = useCallback((newCustomization: BlogCustomization) => {
    const storageKey = getStorageKey();
  ...
  const updateCustomization = useCallback((updates: Partial<BlogCustomization>) => {
