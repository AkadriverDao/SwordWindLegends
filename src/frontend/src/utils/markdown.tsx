...
export function parseMarkdown(content: string): React.ReactNode {
  ...
  const flushParagraph = () => {
    if (currentParagraph.length > 0) {
      ...
      elements.push(
        <p
          key={elements.length}
          className="mb-6 leading-relaxed text-dark-text whitespace-pre-wrap"
        >
  ...
  const flushCodeBlock = () => {
    if (codeBlockContent.length > 0) {
      elements.push(
        <pre
          key={elements.length}
          className="bg-dark-surface border-2 border-gray-500 rounded-xl p-4 mb-6 overflow-x-auto shadow-lg"
        >
  ...
  lines.forEach((line, _index) => {
    ...
    if (codeBlockMatch) {
      if (inCodeBlock) {
        ...
        flushParagraph();
        inCodeBlock = true;
        _codeBlockLanguage = codeBlockMatch[1] || "";
      }
      return;
    }
    ...
    if (line.startsWith("### ")) {
      ...
      elements.push(
        <h3
          key={elements.length}
          className="text-2xl font-bold text-dark-text mb-4 mt-8 border-b-2 border-theme-primary/30 pb-2"
        >
    ...
    if (line.startsWith("## ")) {
      ...
      elements.push(
        <h2
          key={elements.length}
          className="text-3xl font-bold text-dark-text mb-5 mt-10 border-b-2 border-theme-primary/50 pb-3"
        >
    ...
    if (line.startsWith("# ")) {
      ...
      elements.push(
        <h1
          key={elements.length}
          className="text-4xl font-bold text-dark-text mb-6 mt-12 border-b-4 border-theme-primary pb-4"
        >
    ...
    if (line.startsWith("> ")) {
      ...
      elements.push(
        <blockquote
          key={elements.length}
          className="border-l-4 border-theme-primary pl-6 py-3 mb-6 bg-theme-primary/10 rounded-r-xl shadow-lg"
        >
    ...
    if (
      ...
      elements.push(
        <hr
          key={elements.length}
          className="border-t-2 border-gray-500 my-8"
        />,
      );
      return;
    }
  ...
  return (
    <div className="markdown-rendered prose prose-lg max-w-none">
      {elements}
    </div>
  );
}
...
function parseInlineMarkdown(text: string): React.ReactNode {
  ...
  const processText = (inputText: string): React.ReactNode[] => {
    ...
    const patterns: MarkdownPattern[] = [
        ...
        render: (match: RegExpMatchArray) => {
          ...
          return (
            <img
              ...
              src={url}
              alt={alt}
              className="max-w-full rounded-xl my-6 shadow-lg border-2 border-gray-500"
              loading="lazy"
            />
          );
        },
        ...
        render: (match: RegExpMatchArray) => {
          ...
          if (
            ...
            if (
              ...
              finalUrl = linkText.startsWith("http")
                ? linkText
                : `https://${linkText}`;
            } else {
      ...
      // Strikethrough: ~~text~~
      {
        ...
        render: (match: RegExpMatchArray) => (
          <span
            key={key++}
            className="line-through text-dark-text-secondary decoration-2"
          >
            {processText(match[1])}
          </span>
        ),
      ...
      // Inline code: `code` - process last to avoid conflicts
      {
        ...
        render: (match: RegExpMatchArray) => (
          <code
            key={key++}
            className="bg-dark-surface border border-gray-500 rounded-md px-2 py-1 font-mono text-sm text-green-400 shadow-sm"
          >
            {match[1]}
          </code>
        ),
    ...
    for (const pattern of patterns) {
      ...
      while (true) {
        match = pattern.regex.exec(processedText);
        if (match === null) break;
