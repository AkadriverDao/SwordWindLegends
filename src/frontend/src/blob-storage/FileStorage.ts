...
import { FileReference } from "../types/backend";
import { StorageClient } from "./StorageClient";

const getHttpAgent = async () => {
  const host = import.meta.env.VITE_BACKEND_HOST || "http://localhost:4943";
...
export const useFileUrl = (path: string) => {
  ...
  const getFileReference = async (path: string) => {
    if (!actor) throw new Error("Backend is not available");
    const bucketName = import.meta.env.VITE_STORAGE_BUCKET_NAME || "default";
    const storageGatewayUrl = import.meta.env.VITE_STORAGE_GATEWAY_URL || "";
