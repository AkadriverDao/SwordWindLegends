...
import type { FileReference } from "../types/backend";
...
export class StorageClient {
  ...
  private async getCertificate(hash: string): Promise<Uint8Array> {
    ...
    if (response && isV4ResponseBody(response)) {
  ...
  public async putFile(
    ...
    // Cast to any since backendInterface is empty but runtime actor has these methods
    const actorAny = this.actor as any;
    await actorAny.registerFileReference(path, hashString);
  ...
  public async listObjects(): Promise<FileReference[]> {
    // Cast to any since backendInterface is empty but runtime actor has these methods
    const actorAny = this.actor as any;
    return await actorAny.listFileReferences();
  }
  ...
  public async getDirectURL(path: string): Promise<string> {
    ...
    // Cast to any since backendInterface is empty but runtime actor has these methods
    const actorAny = this.actor as any;
    const fileReference = await actorAny.getFileReference(path);
