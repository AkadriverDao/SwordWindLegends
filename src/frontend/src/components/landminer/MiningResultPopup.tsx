import { AlertTriangleIcon, CoinsIcon, XIcon } from "lucide-react";
...
export default function MiningResultPopup({
  result,
  currentGoldBalance,
  onClose,
}: MiningResultPopupProps) {
  ...
  const getResultIcon = () => {
    switch (result.outcome) {
      ...
      case "dark_matter":
        return (
          <span className="text-6xl animate-bounce dark-matter-warning">
            💀
          </span>
        );
    }
  ...
  const modalContent = (
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4 z-modal">
      <div
        className={`bg-dark-card backdrop-blur-sm rounded-3xl shadow-2xl border-4 border-gray-500 w-full max-w-md animate-bounce-fun ${
          result.outcome === "gold"
      ...
      >
        {/* Header */}
        <div
          className={`p-6 border-b-4 border-gray-500 bg-gradient-to-r ${getResultColor()} rounded-t-3xl ${
        ...
        >
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h3 className="text-xl font-bold text-white global-font">
                  ⛏️ 挖矿结果
                </h3>
                <p className="text-white/90 font-medium global-font">
                  土地 #{result.landId}
                </p>
              </div>
            </div>
        ...
        >
          <div className="text-center">
            <p className="text-dark-text-secondary text-sm font-medium global-font">
              {result.outcome === "gold" &&
                `🎉 继续挖矿获得更多金币！每块土地都可以多次挖矿！您的金币余额已实时更新至 ${currentGoldBalance} 枚，请查看游戏控制台！`}
              {result.outcome === "empty" &&
                "💪 不要放弃，再次挖矿一定会有奖励！"}
              {result.outcome === "dark_matter" &&
                "⚠️ 挖矿有风险，探索需谨慎！复活后可以重新开始！"}
            </p>
          </div>
        </div>
      </div>
