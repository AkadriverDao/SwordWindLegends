...
export default function UnifiedGameStatus({
  ...
  const [previousGoldAmount, setPreviousGoldAmount] = useState<number | null>(
    null,
  );
  ...
  const currentGoldCoins =
    playerGold !== undefined ? playerGold : gameState?.goldCoins || 0;
  ...
  const handleMineLand = async () => {
    ...
    try {
      ...
      const errorMessage =
        error instanceof Error ? error.message : "挖矿失败，请重试";
  ...
  const handleReviveCharacter = async () => {
    ...
    try {
      ...
      const errorMessage =
        error instanceof Error ? error.message : "复活失败，请重试";
      setOperationError(errorMessage);
    ...
    <div className="bg-dark-surface rounded-2xl border-3 border-gray-500 p-6">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              <HeartIcon
                size={20}
                className={
                  gameState.isAlive ? "text-green-500" : "text-red-500"
                }
              />
            ...
            <div className="flex items-center space-x-3">
              ...
              <div
                className={`bg-gradient-to-r from-yellow-400 to-orange-400 border-2 border-yellow-500 rounded-xl px-4 py-2 shadow-lg transition-all duration-500 ${
                  showGoldIncrease
                    ? "scale-110 shadow-2xl ring-4 ring-yellow-300 animate-pulse"
                    : ""
                }`}
              >
                <div className="text-center">
                  <div
                    className={`text-2xl font-bold text-white elegant-numbers flex items-center space-x-1 ${
        ...
        <div className="mb-4">
          <div className="bg-dark-card rounded-xl p-3 border-2 border-gray-500">
            ...
            <button
              ...
              disabled={
                isMiningLand || isLoading || isMining || isCharacterDead
              }
      ...
      <div className="grid grid-cols-3 gap-2 mb-4">
        ...
        {gameState && (
          <>
            ...
            <div
              className={`bg-gradient-to-r from-yellow-400/20 to-orange-400/20 border-2 border-yellow-500 rounded-lg p-2 text-center shadow-lg transition-all duration-500 ${
                showGoldIncrease
                  ? "scale-110 shadow-2xl ring-2 ring-yellow-300 bg-gradient-to-r from-yellow-400/40 to-orange-400/40"
                  : ""
              }`}
            >
              <div
                className={`text-xl font-bold text-yellow-500 elegant-numbers flex items-center justify-center space-x-1 ${
                  showGoldIncrease ? "animate-bounce" : ""
                }`}
              >
                <CoinsIcon
                ...
                />
                <span className={showGoldIncrease ? "gold-celebration" : ""}>
                  {currentGoldCoins}
                </span>
                {showGoldIncrease && (
              ...
              <div
                className={`text-xs global-font font-bold ${
                  showGoldIncrease
          ...
          <div className="flex items-center justify-center space-x-3">
            ...
            <div className="text-center">
              ...
              <p className="text-white/90 text-sm global-font">
                +{currentGoldCoins - previousGoldAmount} 金币 → 总计{" "}
                {currentGoldCoins} 金币
              </p>
      ...
      {gameStatistics && (
        <div className="mb-4">
          <div className="flex items-center space-x-2 mb-2">
            ...
            <h4 className="text-sm font-bold text-dark-text global-font">
              📊 服务器统计
            </h4>
          </div>
      ...
      <div className="bg-gradient-to-r from-theme-primary/20 to-theme-secondary/20 rounded-lg p-3 border border-gray-500">
        <div className="flex items-start space-x-2">
          ...
          <div>
            <p className="text-dark-text font-bold global-font text-xs mb-1">
              游戏操作指南
            </p>
            <p className="text-dark-text-secondary text-xs global-font">
              {isCharacterDead
                ? "角色已死亡，金币已清零，请先点击复活按钮恢复生命"
                : isMining
                  ? "正在生成挖矿结果，请等待完成"
