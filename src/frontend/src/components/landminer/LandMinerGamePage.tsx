...
export default function LandMinerGamePage({
  ...
  const [miningCountdown, _setMiningCountdown] = useState<number | null>(null);
  const [miningResult, _setMiningResult] = useState<
    (MiningResult & { landId: number }) | null
  >(null);
  const [showResultPopup, setShowResultPopup] = useState(false);
  const [isMiningInProgress, _setIsMiningInProgress] = useState(false);
  ...
  const _refreshPlayerGold = useRefreshPlayerGold();

  // 主 canister变更操作 - 完全主 canister控制
  const _mineLand = useMineLand();
  const _reviveCharacter = useReviveCharacter();
  const _refreshUserGameState = useRefreshUserGameState();
  ...
  useEffect(() => {
    if (landsError) {
      ...
      setBackendConnectionError(
        "无法连接到主 canister游戏服务器，请检查网络连接或联系管理员",
      );
          ...
          <div className="flex items-center space-x-4">
            ...
            <div className="flex-1">
              ...
              <p className="text-dark-text-secondary font-medium global-font">
                在任意土地上开始您的挖矿冒险之旅！所有挖矿结果由主
                canister安全随机生成，金币余额通过getPlayerGold接口实时更新
              </p>
            </div>
