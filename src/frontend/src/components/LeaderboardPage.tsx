...
import {
  useGetMiningLeaderboard,
  useGetUserProfile,
} from "../hooks/useUserQueries";
import type { MiningLeaderboardEntry } from "../types/backend";
...
export default function LeaderboardPage({ navigate }: LeaderboardPageProps) {
  const { data: miningLeaderboard, isLoading: leaderboardLoading } =
    useGetMiningLeaderboard();
  ...
  const getRankIcon = (rank: number) => {
    switch (rank) {
      ...
      default:
        return (
          <span className="text-gray-500 font-bold text-lg elegant-numbers">
            #{rank}
          </span>
        );
    }
  ...
  const formatGoldAmount = (goldCoins: bigint) => {
    return goldCoins.toString();
  };
...
interface MiningLeaderboardItemProps {
  ...
  formatGoldAmount: (goldCoins: bigint) => string;
}
...
function MiningLeaderboardItem({
  ...
  return (
    <div
      className={
        "bg-dark-surface backdrop-blur-sm rounded-2xl shadow-lg border-3 p-6 transition-all duration-300 hover:shadow-xl hover:scale-[1.02] border-gray-500"
      }
    >
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-4">
          {/* Rank Badge */}
          <div
            className={`w-16 h-16 bg-gradient-to-r ${getRankBadgeColor(rank)} rounded-full flex items-center justify-center shadow-lg border-3`}
          >
            {getRankIcon(rank)}
          </div>
          ...
          <div>
            ...
            <div className="flex items-center space-x-2 text-gray-400">
              <div className="flex items-center space-x-1">
                <CoinsIcon size={16} />
                <span className="font-medium global-font elegant-numbers">
                  {formatGoldAmount(entry.goldCoins)} 枚金币
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="bg-yellow-500/20 border-yellow-500 px-4 py-2 rounded-xl border-2">
            <div className="text-2xl font-bold text-yellow-500 flex items-center justify-center space-x-2 elegant-numbers">
              ...
              <span className="global-font">
                {formatGoldAmount(entry.goldCoins)}
              </span>
            </div>
