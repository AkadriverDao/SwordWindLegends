  ...
  useEffect(() => {
    ...
    try {
      ...
      if (priceData.length === 0) {
        ...
        ctx.fillText(
          isLoading
            ? `正在加载 ${selectedTradingPair} 数据...`
            : `暂无 ${selectedTradingPair} 数据`,
          width / 2,
          height / 2,
        );
        return;
      ...
      if (prices.length === 0) {
        throw new Error(
          `没有有效的 ${selectedTradingPair} 价格数据用于绘制图表`,
        );
      }
      ...
      const getX = (index: number) =>
        (index / (priceData.length - 1)) * chartWidth;
      const getY = (price: number) =>
        ((adjustedMax - price) / adjustedRange) * chartHeight;
      ...
      priceData.forEach((point, index) => {
        if (point && point.price > 0 && Number.isFinite(point.price)) {
          const x = getX(index);
      ...
      for (let i = 0; i <= timeGridLines; i++) {
        const dataIndex = Math.floor(
          (i / timeGridLines) * (priceData.length - 1),
        );
      ...
      if (hoveredPoint) {
        const adjustedX =
          padding.left + viewOffset.x + (hoveredPoint.x - padding.left);
