  ...
  if (!isAuthenticated) {
    return (
      <div className="flex items-center space-x-2 text-dark-text-secondary">
        ...
        <span className="font-medium elegant-numbers global-font">
          {likeCount}
        </span>
      </div>
    );
    ...
    >
      {isLoading ? (
        ...
        <HeartIcon
          size={20}
          className={
            userHasLiked ? "text-white fill-current" : "text-dark-error"
          }
        />
      )}
      <span className="elegant-numbers">{likeCount}</span>
      <span className="hidden sm:inline">{userHasLiked ? "已赞" : "点赞"}</span>
    </button>
  );
}
