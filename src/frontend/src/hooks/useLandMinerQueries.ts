  ...
  return useQuery<GameStatistics | null>({
    ...
    queryFn: async () => {
      ...
      try {
        ...
        const [totalPlayers, totalMiningOperations, totalGoldMined] =
          await Promise.all([
    ...
    mutationFn: async (landId: bigint): Promise<MiningResult> => {
      ...
      try {
        ...
        if (backendResult.__kind__ === "nothing") {
          ...
          backendResult.__kind__ === "gold" &&
          backendResult.gold !== undefined
        ) {
        ...
        if (error instanceof Error) {
          if (
            error.message.includes("角色已死亡") ||
            error.message.includes("dead")
          ) {
            throw new Error("角色已死亡，请先复活后再进行挖矿");
          }
          if (
            error.message.includes("无效的土地ID") ||
            error.message.includes("Invalid land")
          ) {
  ...
  return useMutation({
    mutationFn: async (): Promise<void> => {
      ...
      try {
        ...
        if (error instanceof Error) {
          if (
            error.message.includes("角色已存活") ||
            error.message.includes("already alive")
          ) {
            throw new Error("角色已存活，无需复活");
          }
          if (
            error.message.includes("玩家未找到") ||
